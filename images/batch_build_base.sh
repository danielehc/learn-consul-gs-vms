...
DOCKER_BASE_IMAGE=${DOCKER_BASE_IMAGE:-"instruqt-base"}
...
if [ -d "${KEYS_DIR}" ] ; then
  ...
  if [ "$(ls -A ${KEYS_DIR})" ]; then
    ...
    ssh-keygen -t rsa -b 4096 -f ./id_rsa -N "" > /dev/null 2>&1
    popd                                        > /dev/null 2>&1
  fi
fi
...
########## ------------------------------------------------
header     "BUILDING BASE IMAGE"
###### -----------------------------------------------


log "Building \033[1m\033[33m${DOCKER_REPOSITORY}/${DOCKER_BASE_IMAGE}\033[0m"

popd  > /dev/null 2>&1 

########## ------------------------------------------------
header     "BUILDING APPLICATON IMAGES"
###### -----------------------------------------------

pwd

for i in `find ./ -maxdepth 1 -type d -name "base-*"`; do
  log_err $i
done

# log "Checking application versions"
